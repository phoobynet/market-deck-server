...
type SnapshotLiteStream struct {
	mu                     sync.RWMutex
	deckRepo               *decks.DeckRepository
	snapshotsRepo          *Repository
	publishDuration        time.Duration
	publishTicker          *time.Ticker
	snapshotsLite          cmap.ConcurrentMap[string, SnapshotLite]
...
func NewSnapshotLiteStream(
	...
	s := &SnapshotLiteStream{
		...
		snapshotsLite:          cmap.New[SnapshotLite](),
...
func (s *SnapshotLiteStream) UpdateSymbols(symbols []string) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *SnapshotLiteStream) removeSymbols(removedSymbols []string) {
...
func (s *SnapshotLiteStream) addSymbols(addedSymbols []string) {
...
func (s *SnapshotLiteStream) refreshSnapshots() {
...
func (s *SnapshotLiteStream) updateDeck(symbols []string) {
	if len(symbols) == 0 {
